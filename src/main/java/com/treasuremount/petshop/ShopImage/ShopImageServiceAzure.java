...
@Service
@Slf4j
public class ShopImageServiceAzure {

    private static final Logger logger = LoggerFactory.getLogger(ShopImageServiceAzure.class);

    @Autowired
    private ShopImagesRepository productImagesRepository;
    ...
    @Autowired
    public ShopImageServiceAzure(BlobContainerClient blobContainerClient) {
    ...
    public void deleteImages(Long productId, Long positionId) {
        if (positionId == 0L) {
            List<ShopImages> imagesList=productImagesRepository.findByProductId(productId);
            for (ShopImages image : imagesList) {
...
//                productImagesRepository.delete(image);
            ...
            List<ShopImages> images = productImagesRepository.findByProductIdAndPositionId(productId, positionId);
    ...
    public List<ShopImages> uploadImages(Long productId, Long positionId, MultipartFile file) throws IOException {
     ...
     /*   Path uploadPath = Paths.get(uploadDir, "product_images", String.valueOf(productId)).toAbsolutePath().normalize();
        ...
        String location="ShopImages/"+String.valueOf(productId);
...
//        file.transferTo(filePath);
        ...
        String fileDownloadUri = ServletUriComponentsBuilder.fromHttpUrl("https://petsshopapi-d6dccjc9bne5g7ce.centralindia-01.azurewebsites.net/")
                .path("/api/public/ShopImages/files/")
                .path(uniqueFileName)
                .toUriString();

        // Check if there's an existing image with the same positionId for the given productId
        List<ShopImages> existingImages = productImagesRepository.findByProductIdAndPositionId(productId, positionId);

        if (!existingImages.isEmpty()) {
            // If an image exists, delete the old image from the filesystem and update the record
            ShopImages existingImage = existingImages.get(0);
            ...
            ShopImages productImage = new ShopImages();
            productImage.setVendorId(productId);
...
*/
    ...
    public void deleteImageFile(String imageUrl) {
        try {
            ...
            String subDir = "ShopImages/" + productId ;
    ...
    public ResponseEntity<byte[]> serveFile(String fileName) {
        try {
            ...
            String location = "ShopImages/" + productId ;
